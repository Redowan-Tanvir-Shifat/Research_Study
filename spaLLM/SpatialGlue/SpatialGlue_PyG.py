...
from .SpatialGlue_model import Encoder_overall
...
class Train_SpatialGlue:
    def __init__(self,
        ...
        self.embedding = torch.from_numpy(embedding).to(self.device)  # (3484, 512)
        ...
        self.adj = adjacent_matrix_preprocessing(self.adata_omics1, self.adata_omics2, self.data['adj_emb'])
        ...
        self.adj_emb = self.adj['adj_emb'].to(self.device)
    ...
    def train(self, epochs=100):
        """print("dim: in1, out1, in2, out2")   # 30 64 30 64
        print(self.dim_input1)
        print(self.dim_output1)
        print(self.dim_input2)
        print(self.dim_output2)
        exit()"""
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if random.random() < 0:
                ...
                cellembedding_noisey = add_gaussian_noise(self.embedding, mean=0, std=0.01)
                results = self.model(features_omics1_noisy, self.features_omics2, self.adj_spatial_omics1,
                                     self.adj_feature_omics1, self.adj_spatial_omics2, self.adj_feature_omics2,
                                     cellembedding_noisey, self.adj_emb)
            else:
        ...
        # self.plot_loss()   # print loss curve
