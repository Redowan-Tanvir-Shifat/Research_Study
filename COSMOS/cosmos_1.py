'''
This open-source software is for implementing the COSMOS algorithm with GAT-V2 encoder. 
Paper: Cooperative Integration of Spatially Resolved Multi-Omics Data with COSMOS

Modified Version: GAT-V2 Encoder Variant
Original Paper: Cooperative Integration of Spatially Resolved Multi-Omics Data with COSMOS
...
Version: 10/10/2024 (Modified with GAT-V2)
...
class Cosmos(object):
    """An object for analysis of spatial transcriptomics data with GAT-V2 encoder.
    ...
    def train(self, embedding_save_filepath="./embedding.tsv", weights_save_filepath="./weights.tsv", spatial_regularization_strength=0.05, z_dim=50, lr=1e-3, wnn_epoch  = 100, total_epoch = 1000, max_patience_bef=10, max_patience_aft=30, min_stop=100, random_seed=42, gpu=0, regularization_acceleration=True, edge_subset_sz=1000000):
        ...
        """
        Training the Deep GraphInfomax Model with GAT-V2 Encoder
...
class GraphEncoderWNNit(nn.Module):
    """GAT-V2 based encoder for dual-stream multi-omics integration"""
    def __init__(self,nsample, in_channels1, in_channels2, hidden_channels):
        super(GraphEncoderWNNit, self).__init__()
        # GAT-V2 layers for first modality
        self.gat1 = GATv2Conv(in_channels1, hidden_channels, heads=4, concat=True)
        self.gat3 = GATv2Conv(hidden_channels * 4, hidden_channels, heads=4, concat=True)
        
        # GAT-V2 layers for second modality
        self.gat2 = GATv2Conv(in_channels2, hidden_channels, heads=4, concat=True)
        self.gat4 = GATv2Conv(hidden_channels * 4, hidden_channels, heads=4, concat=True)
        
        # Activation functions
        self.prelu = nn.PReLU(hidden_channels * 4)
        self.prelu2 = nn.PReLU(hidden_channels * 4)
        self.prelu3 = nn.PReLU(hidden_channels * 4)
        self.prelu4 = nn.PReLU(hidden_channels * 4)
        
        # Linear projection layers to reduce dimensions after GAT attention heads
        self.proj1 = nn.Linear(hidden_channels * 4, hidden_channels)
        self.proj2 = nn.Linear(hidden_channels * 4, hidden_channels)
        
        self.mddim = hidden_channels

    def forward(self, x1, x2, edge_index, adata, w, w1, w2):
        # Process first omics modality with GAT-V2
        x1 = self.gat1(x1, edge_index)
        x1 = self.prelu(x1)
        x1 = self.gat3(x1, edge_index)
        x1 = self.prelu2(x1)
        x1 = self.proj1(x1)  # Project back to hidden_channels
        x1 = nn.functional.normalize(x1, p=2.0, dim=1)

        # Process second omics modality with GAT-V2
        x2 = self.gat2(x2, edge_index)
        x2 = self.prelu3(x2)
        x2 = self.gat4(x2, edge_index)
        x2 = self.prelu4(x2)
        x2 = self.proj2(x2)  # Project back to hidden_channels
